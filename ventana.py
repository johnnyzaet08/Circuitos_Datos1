...
class Ventana:
    ...
    def modifica(self,x,y,resistencia,btnResistencia):
        if resistencia.active == False:
            resistencia.active = True
            canvas = Canvas(self.instancia , width= 120, height = 40, bg = "#eeaa02")
            btnGirarImage = self.img_Girar
            btnEliminarImage = self.img_Girar
            btnCerrarImage = self.img_Girar
            btnGirar = Button(canvas,image = btnGirarImage, command = lambda:self.girarResistencia(resistencia,btnResistencia))
            btnGirar.place(x= 0,y=0)
            btnEliminar = Button(canvas,image = btnEliminarImage, command = lambda:self.eliminarComponente(btnResistencia,canvas))
            btnEliminar.place(x= 40,y=0)
            btnCerrar = Button(canvas,image = btnCerrarImage, command = lambda:self.cierraCanvas(resistencia, canvas))
